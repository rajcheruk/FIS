...
public class gaussianFilter {
    ...
    public void generateTemplate() {
        ...
        for (int x = 0; x < templateSize; x++) {
            for (int y = 0; y < templateSize; y++) {
                template[x * templateSize + y] = (1 / (float) (2 * Math.PI * sigma * sigma)) * (float) Math.exp((-((x - center) * (x - center) + (y - center) * (y - center)) / (2 * sigma * sigma)));
                total += template[x * templateSize + y];
            }
        }
        for (int x = 0; x < templateSize; x++) {
            for (int y = 0; y < templateSize; y++) {
                template[x * templateSize + y] /= total;
            }
        }
    }
    ...
    private static final Logger LOG = Logger.getLogger(gaussianFilter.class.getName());
